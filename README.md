...
At the end, CAPPS will name the files by the order at which they were loaded. If you choose to reference the file number to your original name tags, please click on the **_Experiment Name Assigned_ _Exp_Settings.txt** file, and it will trace the file numbers to your original naming scheme. 
...
The output for CAPPS consists of the following files:

**_GENERAL FILES_**
1. Experiment settings text file: This consists of the settings that was picked by the user
2. Log file (.txt): This file consists of the steps that were performed by CAPPS. If an error is generated during filtering, it will be logged in the **Log file.txt**

&nbsp;

**`.bam` SPECIFIC FILES**
1. `.bw` file: This is the BigWig file that can be used for viewing peak summit tracks in IGV or pyGenomeTracks 
2. A peak folder: This folder contains the MACS2 output 
3. A `.bam` file: This is the `.bam` generated by CAPPS, which has been:
   - filtered for map Quality score
   - filtered for mitochondrial reads
   - filtered for read duplicates
   - Sorted
   - Indexed
4. Fragment Size distribution Graph: This is the fragment size distribution graph generated using **_picards.jar_**
5: A `.bed` file: This is the file used for differential peak analysis in CAAT (Based in Shiny Dashboard).
6: `.bai` file: This is an index file that is generated when the `.bam` file is sorted. 

&nbsp;

In conclusion, **CAPPS** provides a way to automate filtering, peak calling, and BigWig file generation for ATAC-seq and ChIP-seq `.bam` that does not involve the user having access to programming skills or command line. The goal of **CAPPS** is to provide scientists without a programming skill set the ability to analyze their own data remotely and with ease. If you find **CAPPS** useful, please STAR this repository. Thank you. 

&nbsp;

**I hope you find CAPPS useful. Please also check out CAAT, which uses the output `.bam` and `.bed` files generated by CAPPS for differential peak analysis in R. CAAT is written in shiny dashboard.**
